...
// promisify
const writeFile = util__default.promisify(fastWriteAtomic);
const readFile = util__default.promisify(fs.readFile);
const unlink$1 = util__default.promisify(fs.unlink);
...
class Config {
  static async new(state, schema = defaultSchema) {
    await makeDir_1(path.dirname(state.cache_file));
...
async function createSignatureFile(signatureFile, signature) {
  ...
  await makeDir_1(path.dirname(signatureFile));
...
async function check(state) {
  ...
  // format the URL
  const u = Url.parse(state.endpoint, true);
  ...
  // send the request
  const response = await fetch(Url.format(u), {
    method: 'get',
